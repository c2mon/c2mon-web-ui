...
import cern.c2mon.web.ui.process.ProcessService;
...
public class TagController implements TagListener {
  ...
  public Object getTag(@PathVariable final String id, WebRequest request) throws IOException {
    Tag tag;

    if (StringUtils.isNumeric(id)) {
      tag = tagService.get(Long.valueOf(id));
    } else {
      // The tagname may have slashes inside, so deal with it
      ...
      String tagname = path.substring("/api/tags/".length(), path.length());

      tag = ((List<Tag>) tagService.findByName(tagname)).get(0);
    }
